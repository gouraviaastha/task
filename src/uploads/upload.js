...
const mymodel = require('../models/file');
...
uploadfile.get('/search/:id', auth,async(req,res)=>{
    const id = req.params.id;
    const path = req.body.logicpath;
    try{
        const file = await mymodel.find({userId:id,logicpath:path})
        if(id!==req.userId){
...
uploadfile.get('/searchbyname/:id/:name', auth,async(req,res)=>{
    const id = req.params.id;
    const file_name = req.params.name;
    try{
        const file = await mymodel.find({userId:req.params.id,file:file_name})
        if(id!==req.userId){
            res.status(200).send({msg : "Invalid User"})
            }
        else if (file.length == 0 ){
